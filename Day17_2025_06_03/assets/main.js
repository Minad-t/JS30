...
function timKiemSanPham(event) {
  event.preventDefault();

  const search = document
    .getElementById("nut-timkiem")
    .value.trim()
    .toLowerCase();
  const tbody = document
    .getElementById("bang-sanpham")
  ...
  if (!search) {
    alert("Vui lòng nhập ID để tìm kiếm Sản phẩm");
    return;
  }
  ...
  for (let i = 0; i < row.length; i++) {
    const dong = row[i];
    const idTimKiem = dong.cells[0].innerText.toLowerCase();
...
function hienThiDanhSachSanPham() {
  ...
  document.getElementById("nut-timkiem").value = "";
}